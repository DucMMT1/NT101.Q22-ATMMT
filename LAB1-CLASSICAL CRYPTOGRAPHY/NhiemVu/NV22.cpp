...
// Hàm chuẩn hóa: Chuyển văn bản thành chữ hoa và bỏ các ký tự thừa
...
void analyzeFrequency(string text)
{
    ...
    vector<pair<char, int>> sortedList(freq.begin(), freq.end());
    sort(sortedList.begin(), sortedList.end(), [](const auto& a, const auto& b)
    {
        return a.second > b.second;
    });

    cout << left << setw(10) << "Ky tu" << setw(10) << "So lan" << "Ty le (%)" << endl;
    for (auto const& [ch, count] : sortedList)
    {
        double percentage = (double)count / text.length() * 100;
        cout << left << setw(10) << ch << setw(10) << count
             << fixed << setprecision(2) << percentage << "%" << endl;
    }
}

// Hàm giải mã
string monoDecrypt(string ciphertext, map<char, char> key)
{
    for (char &c : ciphertext)
    {
        if (isalpha(c))
        {
            ...
            // Nếu ký tự có trong bảng khóa, tiến hành thay thế
            if (key.count(upperC))
            {
                c = isupper(c) ? key[upperC] : tolower(key[upperC]);
            }
        }
    }
    return ciphertext;
...
Rm xzmxofsrzm, Ervgmln Mlgrzmlo Fmrevisrgb - Hz Xhr Nrmh Xrgb sglmws ls l ovlwrmt vwfxlgrzmlo rmsgrgfgrzm rm Ervgmln, xzmgiryfgrmt srtmrdrxlmgob gz ghv xzfmgib’s rmgvoovxgflo, sxrvmgrdrx, lmw vxzmznrx kiztivss. Rgs xznnrgnvmg gz lxlwvnrx vjxvoovmxv, ivsvlixh rmmzelgrzm, lmw szxrvglo rnklxg hls kzsrgrzmvw rg ls l xifxrlo kolbvi rm shlkrmt ghv dfgfiv zd Ervgmln lmw ghv yizlwvi ivtrzm. Ls rg xzmgrmfvs gz vezoev lmw vjklmw, EMF-HXN ivnlrms wvwrxlgvw gz dzsgvirmt ghv mvjg tvmvilgrzm zd ovlwvis lmw rmmzelgzis uhz uroo wirev sfsglrmlyov tizugh lmw tozylo kiztivss.)";
    // 1. Thống kê tần suất
    string cleantext = normalize(ciphertext);
    analyzeFrequency(cleantext);
    ...
    //3. In ra kết quả giải mã
    cout << "\n--- KET QUA GIAI MA ---" << endl;
    cout << monoDecrypt(ciphertext, myKey) << endl;
